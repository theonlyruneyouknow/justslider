...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateRange(minEl, maxEl, rangeEl, minValueEl, maxValueEl) {
        ...
        minValueEl.textContent = `$${minVal.toFixed(2)}`;
        maxValueEl.textContent = `$${maxVal.toFixed(2)}`;
    ...
    document.querySelectorAll('.range-min, .range-max').forEach(input => {
        input.addEventListener('input', function () {
            ...
            const minValueEl = wrapper.querySelector('.min-value');
            const maxValueEl = wrapper.querySelector('.max-value');
            updateRange(minEl, maxEl, rangeEl, minValueEl, maxValueEl);
        });
    });
